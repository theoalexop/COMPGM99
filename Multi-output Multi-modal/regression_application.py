...
summation of per modality loss functions (see lines 268-276 and 281-296).
...
class RegressionApplication(BaseApplication):
    ...
    def connect_data_and_network(self,
        ...
        if self.is_training:
            ...
            input_modality = tf.unstack(
                value = net_out,
                num = None,
                axis = -1)

            target_modality = tf.unstack(
                value = ground_truth,
                num = None,
                axis = -1)
            ...
            data_loss_T1 = loss_func(
                  prediction=crop_layer(input_modality[0]),
                  ground_truth=crop_layer(target_modality[0]),
                  weight_map=weight_map)

            data_loss_T2 = loss_func(
                  prediction=crop_layer(input_modality[1]),
                  ground_truth=crop_layer(target_modality[1]),
                  weight_map=weight_map)

            data_loss_PD = loss_func(
                  prediction=crop_layer(input_modality[2]),
                  ground_truth=crop_layer(target_modality[2]),
                  weight_map=weight_map)

            data_loss = data_loss_T1 + data_loss_T2 + data_loss_PD
