...
class CubicResizeLayer(Layer):
    """
    Resize 2D/3D images using ``tf.image.resize_bicubic``
    (without trainable parameters).
    """

    def __init__(self, new_size, name='tricubic_resize'):
        """
        ...
        """
        super(CubicResizeLayer, self).__init__(name=name)
        self.new_size = new_size

    def layer_op(self, input_tensor):
        """
        Resize the image by cubicly interpolating the input
        using TF ``resize_bicubic`` function.
        ...
        resize_b_x = tf.image.resize_bicubic(
        ...
        resize_b_z = tf.image.resize_bicubic(
